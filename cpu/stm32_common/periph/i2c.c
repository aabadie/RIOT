...
static inline void _start(I2C_TypeDef *dev, uint8_t address, uint8_t length, uint8_t rw_flag);
...
int i2c_init(i2c_t dev)
{
    ...
    uint32_t timing = (( (uint32_t)tp.presc << 28) |
                       ( (uint32_t)tp.scldel << 20) |
                       ( (uint32_t)tp.sdadel << 16) |
                       ( (uint16_t)tp.sclh << 8) | tp.scll);
                   ...
                   size_t length, uint8_t flags)
{
    ...
    if (!(flags & I2C_NOSTART)) {
        ...
        _start(i2c, address, length, I2C_READ);
    }
...
static inline void _start(I2C_TypeDef *i2c, uint8_t address,
                          uint8_t length, uint8_t rw_flag)
{
    assert(i2c != NULL);
