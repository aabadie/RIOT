...
void epd_bw_spi_write_cmd(epd_bw_spi_params_t *p, uint8_t cmd, const uint8_t *params, size_t plen)
{
...
void epd_bw_spi_read_cmd(epd_bw_spi_params_t *p, uint8_t cmd, uint8_t *params, size_t plen)
{
    spi_acquire(p->spi, p->cs_pin, SPI_MODE_0, p->spi_clk);
    epd_bw_spi_cmd_start(p, cmd, true);
                    ...
                    uint8_t size_x, uint16_t size_y, epd_bw_spi_entry_mode_t entry_mode)
{
    ...
    memcpy(&dev->controller, controller, sizeof(epd_bw_spi_params_t));
    dev->size_x = size_x;
    dev->size_y = size_y;
    ...
    if (dev->params.busy_pin != GPIO_UNDEF) {
    ...
    if (dev->params.dc_pin == GPIO_UNDEF || gpio_init(dev->params.dc_pin, GPIO_OUT) != 0) {
...
void epd_bw_spi_display_init(epd_bw_spi_t *dev)
{
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_DRIVER_OUTPUT_CONTROL, (uint8_t *)y_data, y_size);
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_BOOSTER_SOFT_START_CONTROL, data, 3);

    data[0] = dev->controller.vcom;
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_WRITE_VCOM_REGISTER, data, 1);

    data[0] = 0x1A; /* 4 dummy line per gate */
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_DUMMY_LINE_PERIOD, data, 1);

    data[0] = 0x08; /* 2 µs per line */
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_GATE_LINE_WIDTH, data, 1);

    data[0] = (uint8_t)dev->entry_mode;
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_DATA_ENTRY_MODE_SETTING, data, 1);
}
...
void epd_bw_spi_activate(epd_bw_spi_t *dev)
{
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_MASTER_ACTIVATION, NULL, 0);
...
void epd_bw_spi_update_full(epd_bw_spi_t *dev)
{
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_MASTER_ACTIVATION, NULL, 0);
    epd_bw_spi_wait(&dev->params, EPD_BW_SPI_WAIT_UPDATE_FULL);
...
void epd_bw_spi_update_part(epd_bw_spi_t *dev)
{
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_DISPLAY_UPDATE_CONTROL_2, data, sizeof data);
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_MASTER_ACTIVATION, NULL, 0);
    epd_bw_spi_wait(&dev->params, EPD_BW_SPI_WAIT_UPDATE_PART);
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_NOP, NULL, 0);
}
...
{
    epd_bw_spi_fill(dev, 0, dev->size_x, 0, dev->size_y, EPD_BW_SPI_COLOR_WHITE);
}
...
void epd_bw_spi_set_area(epd_bw_spi_t *dev, uint8_t x1, uint8_t x2, uint16_t y1, uint16_t y2)
{
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_RAM_X, x_data, sizeof x_data);
    ...
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_RAM_Y, (uint8_t *)y_data, y_size);

    /* Set counters to start positions */
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_RAM_X_ADDR_COUNTER, x_data, 1);
    epd_bw_spi_write_cmd(&dev->params, EPD_BW_SPI_CMD_SET_RAM_Y_ADDR_COUNTER, (uint8_t *)y_data, 2);
}
